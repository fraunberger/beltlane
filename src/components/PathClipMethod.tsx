...
interface PathClipMethodProps {
  className?: string;
}

export const PathClipMethod: React.FC<PathClipMethodProps> = ({ className }) => {
  // Base wave path
  ...
  return (
    <div className={`bg-background p-8 flex justify-center ${className || ''}`}>
      <h2 className="text-2xl font-bold mb-3 text-center">Stroke Width Method</h2>
      <svg
        viewBox="0 0 1200 300"
        className="w-full max-w-[800px] h-auto block"
        role="img"
        aria-label="Beltlane Path Test - Stroke Width Method"
      >
        <defs>
          ...
          {/* Define clipPath for the main path shape */}
        ...
        <g id="ringGroup" className="ring-group">
          {/* Using stroke width with clipping for uniform offsets */}
          <g clipPath="url(#ringClip)">
            {/* Paths with different stroke widths */}
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 35, opacity: 0.1 }}
            />
            
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 30, opacity: 0.15 }}
            />
            
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 25, opacity: 0.2 }}
            />
            
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 20, opacity: 0.3 }}
            />
            
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 15, opacity: 0.4 }}
            />
            
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 10, opacity: 0.6 }}
            />
            
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 6, opacity: 0.8 }}
            />
            
            <path 
              className="wave-path"
              d={wavePath}
              style={{ strokeWidth: 3, opacity: 1.0 }}
            />
          </g>
        ...
        <text x="50%" y="90%" textAnchor="middle" fontSize="16" fill="#1d2f35">
          Using stroke-width with SVG clipping
        </text>
      </svg>
    </div>
  );
