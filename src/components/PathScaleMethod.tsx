...
interface PathScaleMethodProps {
  className?: string;
}

export const PathScaleMethod: React.FC<PathScaleMethodProps> = ({ className }) => {
  // Base wave path
  ...
  return (
    <div className={`bg-background p-8 flex justify-center ${className || ''}`}>
      <h2 className="text-2xl font-bold mb-3 text-center">Scale Transform Method</h2>
      <svg
        viewBox="0 0 1200 300"
        className="w-full max-w-[800px] h-auto block"
        role="img"
        aria-label="Beltlane Path Test - Scale Method"
      >
        <defs>
        ...
        <text x="50%" y="90%" textAnchor="middle" fontSize="16" fill="#1d2f35">
          Using scale transforms for path offsets (current approach)
        </text>
      </svg>
    </div>
  );
...
export default PathScaleMethod; 