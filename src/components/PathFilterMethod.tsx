...
interface PathFilterMethodProps {
  className?: string;
}

export const PathFilterMethod: React.FC<PathFilterMethodProps> = ({ className }) => {
  // Base wave path
  ...
  return (
    <div className={`bg-background p-8 flex justify-center ${className || ''}`}>
      <h2 className="text-2xl font-bold mb-3 text-center">SVG Filter Method</h2>
      <svg
        viewBox="0 0 1200 300"
        className="w-full max-w-[800px] h-auto block"
        role="img"
        aria-label="Beltlane Path Test - Filter Method"
      >
        <defs>
          ...
          {/* Define more dramatic filters for path offsets */}
          ...
          <filter id="offset2" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="dilate" radius="6" in="SourceGraphic" result="thicken" />
          </filter>
          
          <filter id="offset3" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="dilate" radius="9" in="SourceGraphic" result="thicken" />
          </filter>
          
          <filter id="offset4" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="dilate" radius="12" in="SourceGraphic" result="thicken" />
          </filter>
          
          {/* Erode filters (negative offset - contracting inward) */}
          <filter id="offsetNeg1" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="erode" radius="3" in="SourceGraphic" result="thin" />
          </filter>
          
          <filter id="offsetNeg2" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="erode" radius="6" in="SourceGraphic" result="thin" />
          </filter>
          
          <filter id="offsetNeg3" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="erode" radius="9" in="SourceGraphic" result="thin" />
          </filter>
          
          <filter id="offsetNeg4" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="erode" radius="12" in="SourceGraphic" result="thin" />
          </filter>
          
          <filter id="offsetNeg5" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="erode" radius="15" in="SourceGraphic" result="thin" />
          </filter>
          
          <filter id="offsetNeg6" x="-20%" y="-20%" width="140%" height="140%">
            <feMorphology operator="erode" radius="18" in="SourceGraphic" result="thin" />
          </filter>
          ...
          <style type="text/css">
            {`
              ...
              .ring-group {
                ...
                transform: perspective(800px) rotateX(20deg) rotateY(-5deg) rotateZ(0deg) scaleY(0.35) translate(0px, 0px);
              }
            `}
          </style>
        </defs>
        ...
        <g id="ringGroup" className="ring-group">
          {/* Erode filters (inner rings) - more dramatic offsets */}
          <path 
            className="wave-path"
            d={wavePath}
            style={{ opacity: 0.3, strokeWidth: 0.8 }}
            filter="url(#offsetNeg6)"
          />
          
          <path 
            className="wave-path"
            d={wavePath}
            style={{ opacity: 0.4, strokeWidth: 1.0 }}
            filter="url(#offsetNeg5)"
          />
          
          <path 
            className="wave-path"
            d={wavePath}
            style={{ opacity: 0.5, strokeWidth: 1.2 }}
          ...
          <path 
            className="wave-path"
            d={wavePath}
            style={{ opacity: 0.7, strokeWidth: 2.0 }}
          ...
          {/* Original path - thickest in the center */}
          <path 
            className="wave-path"
            d={wavePath}
        ...
        <text x="50%" y="90%" textAnchor="middle" fontSize="16" fill="#1d2f35">
          Using SVG Filters (feMorphology with dilate/erode)
        </text>
      </svg>
    </div>
  );
...
export default PathFilterMethod; 